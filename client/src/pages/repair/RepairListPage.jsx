...
export function RepairListPage() {
  const { data: documents, isLoading } = useRepairList();
  const { create } = useRepairMutations();
  ...
  async function handleCreate(values) {
    ...
    setIsCreating(false);
    navigate(`/repair/documents/${document.id}`);
  }

  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>Ремонт</h1>
          ...
          <tbody>
            ...
            {documents?.map((document) => (
              <tr
                ...
                onClick={() => navigate(`/repair/documents/${document.id}`)}
              >
      ...
      {isCreating && (
        <EntityFormModal
          title="Создать документ ремонта"
          fields={headerFields}
          schema={headerSchema}
          defaultValues={{}}
