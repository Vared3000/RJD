...
- `Workwear ERP Hourly Backup` — каждый час, первый плановый запуск через час
  после установки расписания (проверочный запуск выполняется сразу);
...
Параллельный запуск второго backup запрещён настройкой Планировщика и файловой
блокировкой. При обновлении старое задание `Workwear ERP Daily Backup`
...
Перед созданием дампа система проверяет свободное место во всех трёх каталогах.
Остаток менее 20% даёт предупреждение, а нехватка места для очередного дампа —
ошибку без удаления старых файлов. Текущее состояние атомарно записывается в
`BACKUP_ROOT\status\latest.json`; администратор видит на главной странице дату,
размер и свободное место по каждому ПК и получает уведомление о новой ошибке при
следующем входе.
