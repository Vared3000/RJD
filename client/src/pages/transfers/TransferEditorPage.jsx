    ...
    <Modal title="Добавить позицию" onClose={onClose}>
      <form className={catalogStyles.form} onSubmit={handleSubmit}>
        ...
        {!isLoading && options.length === 0 && (
          <p className={catalogStyles.hint}>На складе-отправителе нет доступных экземпляров.</p>
        )}
        <Select
          id="instanceId"
          label="Экземпляр"
  ...
  return (
    <div className={catalogStyles.page}>
      <div className={catalogStyles.header}>
        <div>
          <h1 className={catalogStyles.title}>Перемещение {document.number}</h1>
      ...
      <div className={styles.summary}>
        <div>
          ...
          <span>{document.fromWarehouse?.name ?? '—'}</span>
        </div>
        <div>
          <span className={styles.label}>Куда</span>
          <span>{document.toWarehouse?.name ?? '—'}</span>
        </div>
        <div>
      ...
      {confirmingPost && (
        <Modal title="Провести документ?" onClose={() => setConfirmingPost(false)}>
          <p className={styles.confirmText}>
            Экземпляры будут перемещены на склад-получатель, документ станет недоступен для
