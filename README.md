...
- [ ] Этап 12 — Печатные формы (в работе): готовы ФПУ-26, приложение 1.5 и
      приложение 1.7 в Excel/PDF; остаются УПД, личная карточка и экспорты
      остальных отчётов
...
Обязательные параметры: `dpoId`, `from`, `to`; `format=xlsx|pdf`. Excel
содержит расчётные формулы и итоги, PDF повторяет шапку и заголовок таблицы
на каждой странице. Для оперативных периодов источник — проведённые выдачи.
Если их нет, модуль использует нормализованные строки импортированных актов,
