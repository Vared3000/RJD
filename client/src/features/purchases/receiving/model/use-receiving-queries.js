...
export function useReceivingMutations(id) {
  ...
  const remove = useMutation({ mutationFn: () => receivingApi.remove(id), onSuccess: invalidate });
  ...
  const post = useMutation({ mutationFn: () => receivingApi.post(id), onSuccess: invalidate });
