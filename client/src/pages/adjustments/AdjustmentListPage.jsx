...
import { headerFields, headerSchema } from '../../features/adjustments/model/header-schema.js';
...
export function AdjustmentListPage() {
  const { data: documents, isLoading } = useAdjustmentList();
  ...
  async function handleCreate(values) {
    ...
    setIsCreating(false);
    navigate(`/adjustments/documents/${document.id}`);
  }

  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>Корректировка</h1>
          ...
          <tbody>
            ...
            {documents?.map((document) => (
              <tr
                ...
                onClick={() => navigate(`/adjustments/documents/${document.id}`)}
              >
      ...
      {isCreating && (
        <EntityFormModal
          title="Создать документ корректировки"
          fields={headerFields}
          schema={headerSchema}
          defaultValues={{}}
