...
- `scripts/backup.ps1` — ежедневная и месячная копия без ограничения срока хранения и копирование на NAS;
...
дампов отключено. Логи старше 90 дней удаляются. Ошибка даёт ненулевой код задания, запись в
