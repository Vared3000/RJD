...
- `scripts/backup.ps1` — ежедневная и месячная копия, ротация и копирование на NAS;
...
BACKUP_SECONDARY_PATH=\\backup-server\workwear-erp
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  ...
  -RequireSecondary -NotifyOnFailure -ForceMonthly
...
По умолчанию задания работают от `SYSTEM`. Для NAS в домене выдайте право
записи учётной записи компьютера либо установите задания под отдельной
...
в Windows. В недоменной сети `SYSTEM` не всегда имеет доступ к NAS, поэтому предпочтительна
отдельная локальная служебная учётная запись с доступом к сетевой папке.

Backup хранит последние 30 ежедневных и 24 месячные копии. Логи старше 90
дней удаляются. Ошибка даёт ненулевой код задания, запись в
`BACKUP_ROOT\logs` и, при `-NotifyOnFailure`, сообщение активному пользователю.
