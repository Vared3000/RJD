...
export function WriteoffListPage() {
  const { data: documents, isLoading } = useWriteoffList();
  const { create } = useWriteoffMutations();
  ...
  async function handleCreate(values) {
    ...
    setIsCreating(false);
    navigate(`/writeoff/documents/${document.id}`);
  }

  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>Списание</h1>
          ...
          <tbody>
            ...
            {documents?.map((document) => (
              <tr
                ...
                onClick={() => navigate(`/writeoff/documents/${document.id}`)}
              >
      ...
      {isCreating && (
        <EntityFormModal
          title="Создать документ списания"
          fields={headerFields}
          schema={headerSchema}
          defaultValues={{}}
